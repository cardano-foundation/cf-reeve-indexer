...
class CardIssuanceServiceTest {
    ...
    void issuesOrgLessCardWithEmptyOrganisation() throws Exception {
        // Org-less card: an EXTERNAL holder with no organisation. organisationId is optional
    ...
    void smuggledRawPrivateKeyUnderPrivateKeyHexFieldIsRejected() throws Exception {
        ...
        // sends the raw key under that exact name must be rejected before assembly.
        JsonNode bad = objectMapper.readTree("""
            {"subject":{"subjectType":"EXTERNAL","organisationId":"o"},
    ...
    void unexpectedRequestFieldIsRejectedByTheStrictAllowlist() throws Exception {
        // Any field outside the accepted shape is refused up front, whatever it is named.
        JsonNode bad = objectMapper.readTree("""
    ...
    @Test
    void exportedCardEmitsFullAttestationBlockIncludingKelAnchorAndCredentialCesr() {
        // The producer contract the platform's import verification consumes: an attested card's
        // exported JSON carries every attestation field, INCLUDING (a) the KEL anchor, which is the
        // whole proof now that nothing is published on-chain, and (b) the full CESR credential chain
        // the importer needs to re-validate the credential itself (it cannot fetch it via the OOBI).
        ...
        attested.setAttestationKelSequence("3");
        attested.setAttestationKelEventSaid("EKelEventSaid");
        attested.setAttestationMetadataLabel("170");
        attested.setAttestationCardDigest("ECardDigest");
        attested.setAttestationPayloadSaid("EPayloadSaid");
        ...
        assertEquals("3", card.at("/attestation/kelSequence").asText());
        assertEquals("EKelEventSaid", card.at("/attestation/kelEventSaid").asText());
        // A STRING, not a number: it is fed to the payload SAID as one, so the importer must rebuild
        // that payload with the identical text.
        assertTrue(card.at("/attestation/metadataLabel").isTextual());
        assertEquals("170", card.at("/attestation/metadataLabel").asText());
        assertEquals("ECardDigest", card.at("/attestation/cardDigest").asText());
        assertEquals("EPayloadSaid", card.at("/attestation/payloadSaid").asText());
        assertEquals("-CESR-credential-chain-", card.at("/attestation/credentialCesr").asText());
    }

    @Test
    void exportedCardNeverCarriesAnOnChainTxHash() {
        // The card ceremony publishes nothing to Cardano — attestation.txHash was removed from the
        // wire format, so an importer can never be handed a tx to trust instead of verifying the
        // KEL anchor. Guards against it being reintroduced.
        IssuedCardEntity attested = existingEntity();
        attested.setAttestationAid("EWalletAid");
        attested.setAttestationKelSequence("3");
        attested.setAttestationKelEventSaid("EKelEventSaid");
        ...
        assertFalse(card.at("/attestation").has("txHash"));
    }
    ...
    // --- issueEntityFromCard: register a full client-built card so a ceremony can attest it ---

 ...
 * remotesign ref replies were found, under live wallet testing (see the platform's {@code
 * keri_attestation} module, which this is ported from), not to reliably carry those extra
 ...
 *
 * <p>Ported near-verbatim from the platform's {@code keri_attestation} module's {@code
 * KeriNotificationCorrelator} — the polling loop (synchronous {@code Thread.sleep}, fine here: the
 * design doc's Part A explicitly keeps the whole ceremony synchronous in the indexer's request
 * thread) and the snapshot-exclude logic are unchanged. Adapted: this app injects an {@code
 * Optional<SignifyClient>} directly (gated by {@code keri.enabled}, same convention as {@link
 * KeriService}/{@link CardAttestationOobiService}) rather than the reference's always-present {@code
 * KeriAttestationClient} wrapper behind a {@code @ConditionalOnProperty}-gated bean.
 */
@Service
...
public class KeriNotificationCorrelator {
    ...
    private Optional<CorrelatedNotification> pollOnceByRoute(List<String> routes, Set<String> excludeNoteIds)
        ...
        for (Notification note : notes) {
            ...
            try {
                Optional<Object> exchange = client.orElseThrow().exchanges().get(noteExnSaid);
                if (exchange.isEmpty()) {

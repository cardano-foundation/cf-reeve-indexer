 ...
 * attestation digest in its KEL.
 *
 * <p><b>Wallet-verified payload shape.</b> This wire shape is a proven, live-wallet-tested contract,
 * not an arbitrary design choice: a real Veridian wallet's {@code processRemoteSignReq} expects a
 * self-addressing payload ({@code i} present <em>before</em> saidifying, {@code d} the SAID of the
 * whole payload) and silently drops (no notification, no UI) anything that doesn't look like one. Do

...
public abstract class MetricExecutor {
    ...
    public Object getData(MetricEnum.SubMetric id, String organisationID, Optional<LocalDate> startDate,
            Optional<LocalDate> endDate) {
        Map<MetricEnum.SubMetric, MetricFunction> metricsNotInitialized = Optional.ofNullable(metrics)
                .orElseThrow(() -> new MetricNotFoundException("Metrics not initialized"));
        MetricFunction metricFunction = metricsNotInitialized.getOrDefault(id,
                (String orgId, Optional<LocalDate> start, Optional<LocalDate> end) -> {
                    throw new MetricNotFoundException(
                            String.format("Metric Function %s not found in %s", id.toString(), name));
                });

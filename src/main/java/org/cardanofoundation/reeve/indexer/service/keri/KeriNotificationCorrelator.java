 ...
 * thread-back check, and no recipient-prefix check: under live wallet testing, a real Veridian
 * wallet's IPEX offer/grant and remotesign ref replies were found not to reliably carry those extra
 ...
 * Optional<SignifyClient>} directly (gated by {@code keri.enabled}, same convention as {@link
 * KeriService}/{@link CardAttestationOobiService}).
 */
@Service
...
public class KeriNotificationCorrelator {
     ...
     *  route at once (e.g. an IPEX offer AND a grant together, who must tell which one actually matched). */
    ...
    /**
     * Polls the agent's notification queue at {@link KeriProperties#getNotificationPollInterval()}
    ...
    public Optional<CorrelatedNotification> awaitByRoute(List<String> routes, Duration timeout,
        ...
        while (true) {
            try {
                ...
                Thread.sleep(keriProperties.getNotificationPollInterval().toMillis());
    ...
    public void markAndDelete(String notificationId) {
        requireKeriEnabled();
        try {
            client.orElseThrow().notifications().mark(notificationId);
            client.orElseThrow().notifications().delete(notificationId);
    ...
    private Optional<CorrelatedNotification> pollOnceByRoute(List<String> routes, Set<String> excludeNoteIds)
        ...
        for (Notification note : notes) {
            ...
            try {
                var exchange = client.orElseThrow().exchanges().get(noteExnSaid);
                if (exchange.isEmpty()) {
    ...
    /**
     * Converts the typed exchange back to the generic map {@link CorrelatedNotification} publishes.
     *
     * <p>The client returns a typed {@code ExchangeResource} now, but every consumer of a correlated
     * notification reads the exn generically — {@code exn.get("a")}, {@code exn.get("d")} — because
     * what is interesting differs per route. Converting here keeps that contract intact instead of
     * retyping every caller around a shape they each only partly care about.
     *
     * <p>Note this used to be an {@code instanceof Map} test. Left as-is against the typed return it
     * would simply never match, and correlation would fail silently rather than loudly.
     */

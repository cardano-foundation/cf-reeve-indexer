...
/**
 * The card-attestation ceremony state machine. Every transition takes the row lock via {@link
 * CardAttestationCeremonyRepository#findByIdForUpdate(UUID)} so a retry bumping {@code
 * attemptGeneration} and a late step-completion reading the pre-bump generation can never
 ...
 *
 * <p>Keeps only the CORE state-machine mechanics: row-locked reads, the {@code (state,
 * attemptGeneration)} CAS, and lazy expiry. There is no per-user identity link, no fast-forward, no
 * {@code bindingVersion}/relink invalidation, no {@code AUTH_BEGIN} step, and no per-user
 * active-ceremony limit — none of those concepts exist in the indexer's single-agent,
 * one-card-one-ceremony shape, which is simpler than a per-user ceremony would need to be. There is
 * also no {@code vavr} dependency here: failures that a caller must react to synchronously are
 * plain exceptions ({@link CardCeremonyNotFoundException}, {@link CardCeremonyInvalidStateException},
 * {@link CardCeremonyExpiredException} — mirroring {@link KeriOobiValidationException}'s style),
 * while the CAS methods ({@link #completeStep}, {@link #failStep}, {@link #updateWaitingStepData})
 * silently no-op (boolean/void return) for a superseded attempt, since there is no way to report
 * that back to whichever caller lost the race.
 */
@Service
...
public class CardCeremonyService {
     ...
     *  separate "consumed" step in the card ceremony) — a completed ceremony must never be
     *  reclassified as expired just because nothing has read it back yet. */
     ...
     * decision is left to the REST layer, not this state machine.
     */
     ...
     *  the transition the first time it is observed past due — expiry is checked lazily, on read,

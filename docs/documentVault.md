...
It extends `_backend-services/cf-reeve-ledger-follower-app`, which already does most of the hard part: it follows the chain with yaci-store, receives `TxMetadataEvent`s, filters metadata label 1447, deserialises the CBOR (`LOBOnChainBatchProcessor` → `MetadataDeserialiser`), and already fetches IPFS content through a configured gateway (`ipfs.gateway`, default `https://ipfs.io/ipfs/`). Today it understands only `type: INDIVIDUAL_TRANSACTIONS`; the Indexer teaches it `type: DOCUMENT` and adds a read API plus its own frontend. It is a **separate deployable** from the Reeve monolith, and stays that way.

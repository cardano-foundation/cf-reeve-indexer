...
export const mapSearchFiltersToRequestBody = (values: SearchFiltersValues & SearchQuickFiltersValues): Omit<PostPublicTransactionsRequestBody, 'organisationId'> => ({
  ...
  minAmountFcy: values.minAmountFcy ? parseFloat(values.minAmountFcy.replace(/,/g, '')) : undefined,
  maxAmountFcy: values.maxAmountFcy ? parseFloat(values.maxAmountFcy.replace(/,/g, '')) : undefined,
  minAmountLcy: values.minAmountLcy ? parseFloat(values.minAmountLcy.replace(/,/g, '')) : undefined,
  maxAmountLcy: values.maxAmountLcy ? parseFloat(values.maxAmountLcy.replace(/,/g, '')) : undefined,

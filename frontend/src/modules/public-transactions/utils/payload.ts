...
export const mapSearchFiltersToRequestBody = (values: SearchFiltersValues & SearchQuickFiltersValues): Omit<PostPublicTransactionsRequestBody, 'organisationId'> => ({
  ...
  minAmount: values.minAmountLcy ? parseFloat(values.minAmountLcy.replace(/,/g, '')) : undefined,
  maxAmount: values.maxAmountLcy ? parseFloat(values.maxAmountLcy.replace(/,/g, '')) : undefined,

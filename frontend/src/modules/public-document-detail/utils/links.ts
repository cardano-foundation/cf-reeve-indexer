...
/** L1 explorer link for an anchor's transaction hash - null when the deployment has no explorer configured. */
export const explorerTxUrl = (txHash: string): string | null => composeUrl(APP_EXPLORER_URL, txHash)

/** Raw IPFS gateway link for an envelope CID - null when the deployment has no gateway configured. */

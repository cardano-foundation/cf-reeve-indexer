import { FormikProvider } from 'formik'

import { LayoutPublic } from 'libs/layout-kit/layout-public/LayoutPublic.component.tsx'
import { useTranslations } from 'libs/translations/hooks/useTranslations.ts'
import { usePublicDashboard } from 'modules/public-data-explorer-financial-overview/hooks/usePublicDashboard.ts'
import { PublicDashboardContent } from 'modules/public-data-explorer-financial-overview/sections/PublicDashboardContent/PublicDashboardContent.component.tsx'

export const ViewPublicDataExplorerFinancialOverview = () => {
  const { t } = useTranslations()

  const { dashboard, data, formik, isFetching } = usePublicDashboard()

  return (
    <>
      <LayoutPublic.Header isPublic>
        <LayoutPublic.Header.Details description={t({ id: 'publicDashboardDescription' })} title={t({ id: 'publicDashboardViewTitle' })} />
      </LayoutPublic.Header>
      <LayoutPublic.Main flexDirection="column" gap={{ xs: 2, sm: 6 }}>
        <FormikProvider value={formik}>
          <PublicDashboardContent dashboard={dashboard} data={data} isFetching={isFetching} />
        </FormikProvider>
      </LayoutPublic.Main>
    </>
  )
}

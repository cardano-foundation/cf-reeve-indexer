...
export const TableBatchReviewApprove = ({
  data,
  pagination,
  sorting,
      ...
      renderCell: (row, { isRowChecked, isRowExpanded }) => {
        ...
        return (
          <Box display="flex" gap={1} justifyContent="flex-end">
            <ModalAction
              aria-label={t({ id: 'modalActionApproveSingleTitle' })}
              message={t({ id: 'modalActionApproveSingleMessage' })}
              primaryActionLabel={t({ id: 'confirm' })}
              secondaryActionLabel={t({ id: 'cancel' })}
              primaryActionOnClick={() => onApproveTransaction(row.id)}
              isModalDisabled={isRowChecked}
              renderButton={({ handleClickOpen, isModalDisabled }) => (
                <ButtonSecondary
                  color="success"
                  size="small"
                  sx={{ color: theme.palette.success.dark, borderColor: theme.palette.success.main, '&:hover': { borderColor: theme.palette.success.dark } }}
                  onClick={handleClickOpen}
                  disabled={isModalDisabled || !hasPermission('transactions', 'review_approve')}
                >
                  {t({ id: 'approve' })}
                </ButtonSecondary>
              )}
            ...
            {isRowExpanded && (
              <ModalAction
                ...
                renderButton={({ handleClickOpen, isModalDisabled }) => (
                  <ButtonSecondary
                    ...
                    onClick={handleClickOpen}
                    disabled={isModalDisabled || !hasPermission('transactions', 'review_reject')}
                  >
                    {t({ id: 'reject' })}
                  </ButtonSecondary>
                )}
              >
                <Box mt={3}>
                  <TableTransactionItemsReject data={row.items} onTransactionRejectionReasonsChange={onTransactionRejectionReasonsChange} isFetching={false} isReview />
                </Box>
              </ModalAction>
            )}
          </Box>
  ...
  return (
    <TableContainer>
      ...
      <TableContainer.Table
        aria-label="batch-review-approve-table"
        initialState={{
          sorting: {
        ...
        sortingMode="server"
        totalRows={data?.batchStatistics.approve}

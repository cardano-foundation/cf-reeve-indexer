...
import { StatusOption, useFilters } from 'libs/ui-kit/components/FiltersBatches/hooks/useFilters'
...
const CollapsableCards = ({ batch }: CollapsableCardsProps) => {
  ...
  const { handleCardApproveClick, handleCardInvalidClick, handleCardPendingClick, handleCardPublishClick } = useBatchRedirection({ batch, isReview: true })
  ...
  return (
    <Grid container px={3} py={2} spacing={2}>
      ...
      <Grid size={{ xs: 6, xl: 'grow' }}>
        <CardCounterPublish count={batchStatistics.publish} onClick={handleCardPublishClick} isReview />
      </Grid>
    </Grid>
  )
}

interface BatchesReviewFilters extends ReturnType<typeof useFilters> {
  filterStatusOptions: StatusOption[]
  preselectedOrganisation?: OrganisationApiResponse
}

interface BatchesReviewPagination {
  handlePagination: ReturnType<typeof usePagination>['handlePagination']
}

interface TableBatchesReviewProps {
  data: BatchesApiResponse | null
  filters: BatchesReviewFilters
  pagination: BatchesReviewPagination
  isFetching: boolean
}

export const TableBatchesReview = ({ data, filters, pagination, isFetching }: TableBatchesReviewProps) => {
  ...
  return (
    <TableContainer>
      ...
      <TableContainer.Table
        aria-label="batches-review-table"
        initialState={{
          sorting: {
        ...
        noRowsHint={t({ id: 'noReviewBatchesHint' })}

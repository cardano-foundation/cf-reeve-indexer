...
import { Forbidden2, Warning2 } from 'iconsax-react'
...
import { getItemViolationsBySource } from 'modules/review/utils/transactions.utils.ts'

interface TableReviewTransactionItemsProps {
  data: TransactionItemApiResponse[] | null
  violations?: Record<string, Violation[]>
  hasReasonsForPending?: boolean
  hasReasonsForRejection?: boolean
  isFetching: boolean
}

export const TableReviewTransactionItems = ({ data, violations, hasReasonsForPending, hasReasonsForRejection, isFetching }: TableReviewTransactionItemsProps) => {
  const { t } = useTranslations()

  const theme = useTheme()

  const columns: GridColDef[] = [
    ...(violations && hasReasonsForPending
      ? [
          {
            field: 'reasonsForPending',
            headerName: '',
            renderCell: ({ row }) => {
              const reasonsForPending = row.rejectionReason ? t({ id: row.rejectionReason }) : getItemViolationsBySource(violations, row.id, ViolationSource.LOB)

              return (
                <Tooltip title={reasonsForPending}>
                  <Box display="flex" alignItems="center" justifyContent="center">
                    {reasonsForPending ? <Warning2 color={theme.palette.action.active} size={20} variant="Outline" /> : null}
                  </Box>
                </Tooltip>
              )
            },
      ...
      : []),
    ...(violations && hasReasonsForRejection
      ? [
          {
            field: 'reasonsForRejection',
            headerName: '',
            renderCell: ({ row }) => {
              const reasonsForRejection = row.rejectionReason ? t({ id: row.rejectionReason }) : getItemViolationsBySource(violations, row.id, ViolationSource.ERP)

              return (
                <Tooltip title={reasonsForRejection}>
                  <Box display="flex" alignItems="center" justifyContent="center">
                    {reasonsForRejection ? <Forbidden2 color={theme.palette.action.active} size={20} variant="Outline" /> : null}
                  </Box>
                </Tooltip>
              )
            },

...
export const UtilisationBar = ({ allocated, spent }: UtilisationBarProps) => {
  ...
  const exceedsFunding = allocated <= 0 && spent > 0
  ...
  return (
    <Box display="flex" flexDirection="column" justifyContent="center" gap={0.5} height="100%" width="100%" pt={1}>
      ...
      />
      <Typography color={overspent ? colors.red[500] : theme.palette.text.secondary} variant="caption" whiteSpace="nowrap">
        {exceedsFunding ? t({ id: 'auditSpendingExceedsFunding' }) : `${Math.round(ratio * 100)}%`}
      </Typography>
    </Box>
  )
}